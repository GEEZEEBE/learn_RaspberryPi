...
void *clnt_connection(void *arg)
{
  ...
  printf("if file_name ->~~~~~~~~~~ %s \n", file_name);
  if(strcmp(file_name, "restful") == 0){	// Send Rest Message
    sendByREST(clnt_write);
  } else {
...
void sendByREST(FILE* fp)
{
  /* 클라이언트로 보낼 실패에 대한 HTTP 응답 메시지 */
  char protocol[] = "HTTP/1.1 200 OK\r\n";
  char server[] = "Server: Netscape-Enterprise/6.0\r\n\r\n";
//  char cnt_len[] = "Content-Length:1024\r\n";
//  char cnt_type[] = "Content-Type: application/json\r\n\r\n";

  /* 화면에 표시될 HTML의 내용 */
  char content1[] = "['id':2172797,'name':'Cairns']";

  printf("send_By_REST API\n");
  fputs(protocol, fp);
  fputs(server, fp);
//  fputs(cnt_len, fp);
//  fputs(cnt_type, fp);
